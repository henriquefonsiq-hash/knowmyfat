...
AI_RULES_FILE = os.path.join(DATA_DIR, "ai_rules.json")
MODEL_NAME = "gemini-flash-latest" # Corrigido após teste de conectividade

# Lógica de API Key (Segurança)
GEMINI_API_KEY = None
...
try:
    ...
    # st.secrets gera erro se não houver arquivo .toml local, ignoramos para usar fallback
    pass
...
if not GEMINI_API_KEY:
    ...
    # 2. Tenta arquivo key.txt na pasta do App
    if not GEMINI_API_KEY:
    ...
    # 3. Tenta diretório interno do PyInstaller (se aplicável)
    if not GEMINI_API_KEY and getattr(sys, 'frozen', False):
        bundle_key = os.path.join(getattr(sys, '_MEIPASS', ''), "key.txt")
        if os.path.exists(bundle_key):
            with open(bundle_key, "r") as f:
                GEMINI_API_KEY = f.read().strip()

if not GEMINI_API_KEY:
    st.error("⚠️ **Erro de Segurança:** A chave de API do Gemini não foi encontrada.")
    st.info(f"""
    Para que a IA funcione, você precisa configurar sua chave:
    1. **Na Nuvem (Streamlit Cloud):** Vá em Settings -> Secrets e adicione `GEMINI_API_KEY = "SUA_CHAVE"`.
    2. **Localmente (PC/Exe):** Crie um arquivo chamado `key.txt` na pasta:
       `{BASE_DIR}` 
...
DEFAULT_ING = [
    {"id": 1, "name": "Frango (Peito Grelhado)", "protein": 31.0, "carbs": 0.0, "fat": 3.6, "calories": calc_kcal(31, 0, 3.6), "category": "Prot", "micro": "Rico em Selênio e B6"},
    {"id": 2, "name": "Arroz Branco Cozido", "protein": 2.5, "carbs": 28.0, "fat": 0.2, "calories": calc_kcal(2.5, 28, 0.2), "category": "Carb", "micro": "Fonte de energia"},
    {"id": 3, "name": "Ovo Cozido", "protein": 13.0, "carbs": 1.1, "fat": 11.0, "calories": calc_kcal(13, 1.1, 11), "category": "Prot/Gord", "micro": "Vitamina D e Colina"},
    {"id": 4, "name": "Azeite de Oliva", "protein": 0.0, "carbs": 0.0, "fat": 100.0, "calories": calc_kcal(0, 0, 100), "category": "Gord", "micro": "Gorduras Saudáveis"},
    {"id": 5, "name": "Feijão Cozido", "protein": 4.8, "carbs": 13.6, "fat": 0.5, "calories": calc_kcal(4.8, 13.6, 0.5), "category": "Prot/Carb", "micro": "Fibras e Ferro"}
]
...
def init_state():
    ...
    if 'ingredients' not in st.session_state:
        if os.path.exists(ING_FILE):
            with open(ING_FILE, "r", encoding="utf-8") as f:
                ings = json.load(f)
                # Garantir que todos tenham campo de calorias
                for i in ings:
                    if 'calories' not in i or i['calories'] == 0:
                        i['calories'] = calc_kcal(i.get('protein', 0), i.get('carbs', 0), i.get('fat', 0))
    ...
    if 'settings' not in st.session_state:
        ...
        if os.path.exists(AI_RULES_FILE):
            with open(AI_RULES_FILE, "r", encoding="utf-8") as f:
                st.session_state['ai_rules'] = json.load(f)
        else:
            st.session_state['ai_rules'] = {
                "methodology": "Você é um nutricionista focado em resultados reais. Use clareza e objetividade.",
                "forbidden_foods": "",
                "preferred_foods": ""
            }
...
tab_diet, tab_foods, tab_progress, tab_bf, tab_training, tab_chat = st.tabs([
    "🍽️ Dieta", "🍎 Alimentos", "📈 Progresso", "🔬 Gordura Corporal", "🎓 Treinamento IA", "💬 Chat Nutricional"
])
    ...
    with st.expander("⚙️ Configuração & Calculadora de Macros", expanded=st.session_state['settings']['target_cals'] == 0):
        ...
        with c_col2:
            ...
            if st.button("Calcular e Aplicar Macros", width="stretch"):
                ...
                st.success("Plano atualizado com macros calculados!")
                st.rerun()

    # --- BOTÃO SUGESTÃO IA ---
    if st.session_state['settings']['target_cals'] > 0:
        st.divider()
        if st.button("🪄 Sugerir Cardápio Completo com IA", use_container_width=True, type="primary"):
            with st.spinner("IA montando sua dieta com base na sua metodologia..."):
                try:
                    client = genai.Client(api_key=GEMINI_API_KEY)
                    extra_knowledge = extract_text_from_pdfs()[:10000]
                    rules = st.session_state['ai_rules']
                    settings = st.session_state['settings']
                    
                    # Formatar a lista de alimentos cadastrados
                    lista_alimentos = ""
                    for ing in st.session_state['ingredients']:
                        cal = ing.get('calories') or calc_kcal(ing.get('protein',0), ing.get('carbs',0), ing.get('fat',0))
                        lista_alimentos += f"- {ing['name']} (P:{ing['protein']}, C:{ing['carbs']}, F:{ing['fat']}, Cal:{cal})\n"
                    
                    prompt = f"""
                    Aja como um nutricionista esportivo seguindo ESTA METODOLOGIA: {rules['methodology']}
                    ALIMENTOS PROIBIDOS: {rules['forbidden_foods']}
                    ALIMENTOS PREFERIDOS: {rules['preferred_foods']}
                    BASE DE CONHECIMENTO EXTRA: {extra_knowledge}
                    
                    --- REGRAS MANDATÓRIAS DE SELEÇÃO ---
                    1. VOCÊ SÓ PODE USAR OS ALIMENTOS DESTA LISTA:
                    {lista_alimentos}
                    2. É TERMINANTEMENTE PROIBIDO SUGERIR QUALQUER ALIMENTO QUE NÃO ESTEJA NA LISTA ACIMA.
                    
                    --- REGRAS DE ARROZ E FEIJÃO ---
                    - Máximo de 2 refeições por dia contendo o combo Arroz + Feijão.
                    - Em qualquer refeição com Arroz e Feijão, a soma das calorias desses dois itens deve ser MENOR que 50% das calorias totais daquela refeição.
                    
                    --- OBJETIVO ---
                    Meta Diária: {settings['target_cals']:.0f} kcal.
                    Macros alvo: P:{settings['p_kg']:.1f}g/kg, C:{settings['c_kg']:.1f}g/kg, F:{settings['f_kg']:.1f}g/kg.
                    
                    Retorne uma lista JSON de 4 refeições (Café, Almoço, Lanche, Jantar).
                    Para cada alimento, use EXATAMENTE o nome que está na lista acima.
                    ...
                    {{
                      "meals": [
                        {{
                          ...
                          "items": [
                            {{"name": "Nome do Alimento EXATO da lista", "protein": 0.0, "carbs": 0.0, "fat": 0.0, "calories": 0.0, "category": "Categoria", "g_suggested": 100}}
                          ]
                        }}
                      ]
                    }}
                    ...
                    if match:
                        ...
                        st.session_state['last_ai_suggestion'] = data
                        st.success("🤖 Sugestão gerada com sucesso!")
                    else:
                        st.error("Erro ao processar resposta da IA. Tente novamente.")
                except Exception as e:
                    st.error(f"Erro ao sugerir dieta: {e}")

        if 'last_ai_suggestion' in st.session_state:
            sug = st.session_state['last_ai_suggestion']
            for m in sug.get('meals', []):
                st.write(f"### 🍴 {m.get('meal_name', 'Refeição')}")
                for item in m.get('items', []):
                    # Uso de .get() para evitar erro se a IA esquecer campos
                    name = item.get('name', 'Alimento')
                    gramas = item.get('g_suggested', 0)
                    prot = item.get('protein', 0.0)
                    carb = item.get('carbs', 0.0)
                    fat = item.get('fat', 0.0)
                    st.write(f"- **{name}**: {gramas}g (P:{prot}g, C:{carb}g, F:{fat}g por 100g)")
            
            if st.button("➕ Adicionar Alimentos Sugeridos à minha Base de Dados"):
                novos = 0
                for m in sug.get('meals', []):
                    for item in m.get('items', []):
                        item_name = item.get('name', 'Novo Alimento')
                        # Evitar duplicados simples por nome
                        if not any(i['name'].lower() == item_name.lower() for i in st.session_state['ingredients']):
                            st.session_state['ingredients'].append({
                                "id": len(st.session_state['ingredients']) + 1,
                                "name": item_name,
                ...
                st.success(f"Sucesso! {novos} novos alimentos adicionados à sua lista.")
                st.rerun()
...
with tab_foods:
            ...
            if st.form_submit_button("Salvar Alimento"):
                if new_name:
                    ...
                    novo_ingrediente = {
                        ...
                        "carbs": new_c,
                        "fat": new_f,
                        "calories": calc_kcal(new_p, new_c, new_f),
                        "category": new_cat,
                        "micro": new_micro
                    }
    ...
    # Tabela editável
    st.subheader("Base de Dados (Editável)")
    st.info("💡 Você pode editar qualquer valor diretamente na tabela abaixo e clicar em 'Salvar'.")
    
    # Garantir que temos um DataFrame sincronizado
    df_sync = pd.DataFrame(st.session_state['ingredients'])
...
with tab_training:
    st.header("🎓 Central de Treinamento da IA")
    st.markdown("""
    Aqui você define a **metodologia** que a IA deve seguir ao sugerir dietas e ao conversar com você.
    Tudo o que você escrever aqui servirá como 'mandamento' para o seu agente.
    """)
    
    with st.form("ai_training_form"):
        methodology = st.text_area("Metodologia Nutricional / Bio", 
                                 value=st.session_state['ai_rules']['methodology'], 
                                 height=200,
                                 help="Descreva sua abordagem: ex: Low Carb, Jejum Intermitente, foco em hipertrofia, etc.")
        
        forbidden = st.text_input("Alimentos a Evitar/Proibir", 
                                value=st.session_state['ai_rules']['forbidden_foods'],
                                help="Separe por vírgula. Ex: Soja, açúcar refinado, glúten.")
        
        preferred = st.text_input("Alimentos Preferidos/Obrigatórios", 
                                 value=st.session_state['ai_rules']['preferred_foods'],
                                 help="Ex: Ovos, abacate, whey protein.")
        
        if st.form_submit_button("💾 Salvar Metodologia", use_container_width=True):
            st.session_state['ai_rules'] = {
                "methodology": methodology,
                "forbidden_foods": forbidden,
                "preferred_foods": preferred
            }
            save_data('ai_rules', AI_RULES_FILE)
            st.success("Metodologia salva! A IA agora seguirá estas regras.")

    st.divider()
    st.subheader("📚 Base de Conhecimento (PDFs)")
    uploaded_pdf = st.file_uploader("Adicionar novo manual/livro (PDF)", type="pdf")
    if uploaded_pdf:
        ensure_books_dir()
        file_path = os.path.join(BOOKS_DIR, uploaded_pdf.name)
        with open(file_path, "wb") as f:
            f.write(uploaded_pdf.getbuffer())
        st.success(f"Arquivo '{uploaded_pdf.name}' adicionado à biblioteca!")
        st.info("A IA lerá este conteúdo automaticamente no próximo chat ou sugestão.")
        ...
        if prompt_user := st.chat_input("Como posso te ajudar hoje?"):
            ...
            with st.chat_message("assistant"):
                try:
                    ...
                    # Pegar contexto da metodologia
                    rules = st.session_state['ai_rules']
                    system_context = f"""
                    Você é um nutricionista virtual. 
                    SUA METODOLOGIA: {rules['methodology']}
                    ALIMENTOS PROIBIDOS: {rules['forbidden_foods']}
                    ALIMENTOS PREFERIDOS: {rules['preferred_foods']}
                    Responda sempre seguindo estes princípios.
                    """
                    
                    # Limitar o contexto para não exceder o limite de tokens (pegando os primeiros 15000 chars como exemplo simples)
                    extra_knowledge = extract_text_from_pdfs()[:15000]
                    
                    full_prompt = f"{system_context}\n\nConhecimento base dos livros: {extra_knowledge}\n\nPergunta do usuário: {prompt_user}"
