...
if "GEMINI_API_KEY" in st.secrets:
    ...
    # Tenta ler de um arquivo key.txt local
        ...
        if st.button("🪄 Sugerir Cardápio Completo com IA", use_container_width=True, type="primary"):
            with st.spinner("IA montando sua dieta com base na sua metodologia..."):
                try:
                    ...
                    prompt = f"""
                    Aja como um nutricionista seguindo esta METODOLOGIA: {rules['methodology']}
                    ...
                    OBJETIVO: Montar uma dieta de {settings['target_cals']:.0f} kcal.
                    Macros alvo: P:{settings['p_kg']:.1f}g/kg, C:{settings['c_kg']:.1f}g/kg, F:{settings['f_kg']:.1f}g/kg.
                    
                    Por favor, sugira uma lista formatada de alimentos para 4 refeições (Café, Almoço, Lanche, Jantar) 
                    que batam aproximadamente esses macros. Retorne apenas a lista de alimentos por refeição.
                    """
                    ...
                    st.info("Você pode copiar estes alimentos para a sua tabela de 'Alimentos' abaixo.")
                except Exception as e:
